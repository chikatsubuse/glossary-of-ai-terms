...
- v3確定後の追加（2026年8月）：FDE（08）＝Palantirカード内の記載を独立化、Prefix Caching（04）、Stripe（11）、ローカルとクラウド（07）、クラウドエージェント（08）
...
| 7 | クラウドエージェント（Cloud Agent） | 追加（2026年8月） |
| 8 | バイブコーディング（Vibe Coding） | |
| 9 | FDE（Forward Deployed Engineer） | 追加（2026年8月）。Palantirカード内の記載を独立化 |
